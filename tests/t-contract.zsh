#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: fasd-simple — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `fasd-simple.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='fasd-simple.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/fasd-simple.plugin.zsh"
    assert $state equals 0
}
